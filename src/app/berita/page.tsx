  ...
  return (
    <div className="py-12 md:py-20 bg-stone-50 min-h-screen">
      <div className="container mx-auto px-4 md:px-6">
        ...
        {allNews.length === 0 ? (
           <div className="text-center py-20 text-stone-500 text-lg">Belum ada berita yang dipublikasikan.</div>
        ) : (
          <>
            {/* Featured News */}
            <div className="mb-16">
              <Link href={`/berita/${allNews[0].slug}`} className="group block">
                <div className="bg-white rounded-3xl overflow-hidden shadow-sm border border-stone-100 grid grid-cols-1 lg:grid-cols-2 group-hover:shadow-lg transition-all">
                  <div className="relative h-64 lg:h-full w-full overflow-hidden bg-stone-100">
                  ...
                  <div className="p-8 md:p-12 flex flex-col justify-center">
                    <div className="flex items-center gap-4 mb-4">
                      <Badge className="bg-teal-500 text-white hover:bg-teal-600">Berita</Badge>
                      <span className="text-sm text-stone-500 flex items-center gap-1">
                        <CalendarDays className="h-4 w-4" /> 
                        {allNews[0].createdAt ? new Date(allNews[0].createdAt).toLocaleDateString('id-ID') : '-'}
                      </span>
                    </div>
                    <h2 className="text-2xl md:text-4xl font-bold text-stone-900 mb-4 group-hover:text-teal-700 transition-colors">
                      {allNews[0].title}
                    </h2>
                    <p className="text-stone-600 text-lg mb-8 leading-relaxed line-clamp-3">
                      {allNews[0].content}
                    </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {allNews.slice(1).map((n) => (
                  <Link href={`/berita/${n.slug}`} key={n.id} className="group block">
                    <Card className="h-full border-stone-100 shadow-sm hover:shadow-md transition-all overflow-hidden bg-white">
                      <div className="relative h-48 w-full overflow-hidden bg-stone-100">
                        ...
                        <div className="absolute top-4 left-4">
                          <Badge className="bg-white/90 text-stone-800 hover:bg-white border-none shadow-sm backdrop-blur-sm">
                            Berita
                          </Badge>
                        </div>
                      </div>
                      <CardContent className="p-6">
                        ...
                        <h3 className="text-xl font-bold text-stone-900 mb-3 group-hover:text-teal-700 transition-colors line-clamp-2">
                          {n.title}
                        </h3>
                        <p className="text-stone-600 text-sm leading-relaxed mb-6 line-clamp-3">
                          {n.content}
                        </p>
                        <div className="text-teal-600 text-sm font-semibold flex items-center gap-1 group-hover:gap-2 transition-all">
