  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-stone-900">Materi Pembelajaran</h2>
          <p className="text-stone-500">Akses dan pelajari modul, presentasi, serta video dari guru-guru.</p>
        </div>
      </div>
        ...
        <CardContent className="p-4 flex flex-col md:flex-row gap-4">
          ...
          <div className="flex gap-2 w-full md:w-auto">
            <Select value={subjectFilter} onValueChange={(val) => setSubjectFilter(val || "all")}>
              <SelectTrigger className="w-full md:w-[200px] h-10 bg-stone-50 border-stone-200">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredMaterials.map((materi) => (
            <Card key={materi.id} className="border-stone-100 shadow-sm hover:shadow-md hover:border-teal-200 transition-all group overflow-hidden flex flex-col">
              <CardHeader className="pb-3 flex flex-row items-start justify-between gap-4">
                ...
                {isNew(materi.createdAt) && (
                  <Badge className="bg-teal-500 hover:bg-teal-600 text-[10px] font-bold px-2 py-0.5">BARU</Badge>
                )}
              </CardHeader>
              <CardContent className="pb-4 flex-1">
                <div className="space-y-1.5">
                  <p className="text-xs font-semibold text-teal-600 uppercase tracking-wider">{materi.subjectName}</p>
                  <h3 className="text-lg font-bold text-stone-900 leading-snug line-clamp-2 group-hover:text-teal-700 transition-colors" title={materi.title}>
                    {materi.title}
                  </h3>
                  {materi.description && (
                    <p className="text-sm text-stone-600 mt-2 line-clamp-2">{materi.description}</p>
                  )}
                  <p className="text-sm text-stone-500 mt-3 flex items-center gap-1.5">
                    <span className="inline-block w-2 h-2 rounded-full bg-stone-300"></span>
                    Oleh {materi.teacherName}
                  </p>
                </div>
              </CardContent>
              <CardFooter className="pt-4 border-t border-stone-50 flex items-center justify-between bg-stone-50/50">
                ...
                <Button size="sm" variant={isVideo(materi.fileUrl) ? 'default' : 'outline'} className={
                  isVideo(materi.fileUrl)
                    ...
                    : 'text-teal-700 border-teal-200 hover:bg-teal-50 shadow-sm'
