    ...
    <div className="py-12 md:py-20 bg-white min-h-screen">
      <div className="container mx-auto px-4 md:px-6">
        ...
        {allGalleries.length === 0 ? (
          <div className="text-center py-20 text-stone-500 text-lg">Belum ada dokumentasi galeri.</div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {allGalleries.map((item) => (
              <div key={item.id} className="group relative rounded-3xl overflow-hidden aspect-square bg-stone-100 shadow-sm cursor-pointer border border-stone-100">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-stone-900/80 via-stone-900/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6">
                  <Badge className="w-fit mb-3 bg-teal-500 hover:bg-teal-500 border-none text-white">
                    Kegiatan
                  </Badge>
                  <h3 className="text-white text-xl font-bold">{item.title}</h3>
                  {item.description && (
                    <p className="text-stone-200 text-sm mt-2 line-clamp-2">{item.description}</p>
                  )}
                </div>
              </div>
            ))}
